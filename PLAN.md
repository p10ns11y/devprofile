...
1. **Hero `#home`** — φ-split cluster (`@container`). Left: `Available now · Stockholm`, name, role. Right: seat + thesis **start-aligned**. Under that, **one packed CTA cluster** (`width: fit-content`) that **edge-hugs** the right cell: text secondaries (View CV · Q&A · Articles) first, **Building primary rightmost** in the pack, social on the same end edge. Never `width: 100%` + `space-between`. Hero is `min-height: 100dvh` so `#about` is not a cut strip on the first screen.
...
| 2 | Right-container CTAs aligned | Packed cluster **edge-hugs** the right cell; text secondaries precede Building (rightmost primary); social shares that end edge |
