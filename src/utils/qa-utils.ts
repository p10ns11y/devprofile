...
export async function prepareData() {
  ...
  dataEntries.push(
    {
      section: "Location",
      content: `I live in ${cvdata.home.current_location}, ${cvdata.home.location}`,
    },
...
export async function generateAnswer(question: string, results: any[]): Promise<string> {
  ...
  const isAchievementsQuestion =
    ...
    /\bmost.*(significant|notable).*contribution\b|\bproudest.*moment\b|\bcareer.*highlight\b/i.test(
      qLower
    );

  const isCareerOverviewQuestion =
    /\bgive.*me.*career.*overview\b|\btell.*me.*career\b|\bcareer.*journey\b|\bwork.*history\b|\bemployment.*history\b/i.test(
      qLower
    ) ||
    /\bprofessional.*path\b|\bcareer.*trajectory\b|\bgrowth.*in.*career\b|\bcareer.*progression\b/i.test(
      qLower
    );

  const isWhyHireQuestion =
    /\bwhy.*hire.*you\b|\bwhy.*should.*hire\b|\bwhy.*you\b|\bwhy.*choose.*you\b|\bwhy.*pick.*you\b|\bwhat.*makes.*you.*good\b/i.test(
      qLower
    ) ||
    /\bwhat.*bring.*to.*table\b|\bwhat.*you.*bring\b|\bwhat.*your.*strength\b|\bunique.*value\b/i.test(
      qLower
    );
  ...
  const isIntroductionQuestion =
    /(\btell.*about.*yourself\b|\bwho.*you\b|\bintroduce.*yourself\b|\bwhat.*do.*you.*do\b|\bwalk.*background\b|\bgive.*overview\b|\byour.*profession\b|\babout.*me\b|\bself.*introduction\b)/i.test(
      qLower
    ) ||
    /\bwhat.*your.*background\b|\bwhat.*you.*bring\b|\byour.*experience\b|\bdescribe.*yourself\b/i.test(
      qLower
    );
  ...
  const isProjectQuestion =
    /\b(projects?.*work|work.*projects?|working.*on.*projects?|current.*projects?|recent.*projects?|which.*projects?|what.*projects?|tell.*about.*projects?|built.*projects?|developed.*projects?)\b/i.test(
      qLower
    ) ||
    /(\bprojects?.*you.*working\b|\bprojects?.*you.*built\b|\bprojects?.*you.*developed\b|\bprojects?.*involved\b)/i.test(
      qLower
    );
...
export function clusterSimilarResults(results: any[]): any[] {
  ...
  results.forEach((result) => {
    ...
    for (const cluster of clusters) {
      ...
      const _combinedText = `${clusterText} ${result.text}`;
  ...
  return clusters
    ...
    .map((cluster) => {
      ...
      return cluster.entries.sort(
        (a: any, b: any) => b.similarity - a.similarity || b.text.length - a.text.length
      )[0];
    });
}
...
export function generateSkillAnswer(
  sectionGroups: Record<string, string[]>,
  _question: string
): string {
  const intro = "Regarding skills and expertise, ";
  const skills: string[] = [];
  ...
  Object.entries(sectionGroups).forEach(([section, texts]) => {
    if (section.includes("Skill") || section.includes("Technolog")) {
      skills.push(
        ...texts.map((t) => t.replace(/^I have (skills|experience) (in|with)/i, "").trim())
      );
    }
  ...
  if (experiences.length > 0) {
    answer +=
      ". Throughout my career, I've applied these skills in various projects, including " +
...
export function generateAchievementsAnswer(cvdata: any): string {
  ...
  const totalYears =
    Math.floor(
      Date.now() -
        new Date(
          cvdata.work_experience[cvdata.work_experience.length - 1].start_date.split(" ").pop() ||
            "2010"
        ).getTime()
    ) /
    (1000 * 60 * 60 * 24 * 365);
  ...
  const notableAchievements = cvdata.work_experience
    .flatMap((exp: any) => exp.responsibilities)
    .filter((resp: string) =>
      /reduced|brought|increased|decreased|achieved|developed|created|implemented|improved|enhanced|optimized/i.test(
        resp
      )
    );

  let answer =
    "I'm particularly proud of several accomplishments in my career. One of my most significant achievements was implementing ";
  ...
  if (notableAchievements.length > 0) {
    ...
    const achievementSummary =
      topAchievement
        .match(
          /(?:developed|created|implemented|improved|enhanced|optimized)(.+?Mean\.|\. |$)/i
        )?.[1]
        ?.trim() || topAchievement;
  ...
  answer +=
    "Another highlight was my role in modernizing legacy codebases and implementing TypeScript integration, which significantly improved our codebase reliability and developer productivity. That's been one of the most impactful contributions I've made in my career.";
...
export function generateCareerOverviewAnswer(cvdata: any): string {
  ...
  const roleProgression = cvdata.work_experience
    .slice(0, 3)
  ...
  answer += `${cvdata.work_experience
    .map((exp: any) => exp.company)
    .slice(0, companyCount >= 3 ? 3 : companyCount)
    .join(", ")}, `;
  answer +=
    "where I've contributed to projects spanning from startup environments to established companies. ";

  // Add skill progression
  answer +=
    "I started working primarily with JavaScript and React, then expanded my expertise to include backend technologies like Python, Node.js, and various databases. ";
  answer +=
    "I also gained deep experience in cloud services and DevOps practices, which helped me take on more senior leadership roles. ";

  // Add future outlook
  answer +=
    "Looking forward, I'm excited to continue growing as a leader in technology and continue contributing to impactful projects that make a difference.";
...
export function generateWhyHireAnswer(cvdata: any): string {
  ...
  let answer =
    "I believe my unique combination of technical expertise, practical experience, and leadership abilities makes me a strong candidate for roles that need both technical depth and strategic thinking. ";
  ...
  answer +=
    "What sets me apart is my ability to bridge technical implementation with business goals. I not only write code that works but architecture that scales, mentor teams that grow, and deliver solutions that create measurable impact. ";
  ...
  answer +=
    "I'm passionate about building applications that users love and teams that excel. I'd bring not just technical skills, but a proven track record of delivery, innovation, and leadership.";
...
export function generateProjectAnswer(sectionGroups: Record<string, string[]>): string {
  ...
  answer +=
    "These projects have spanned various domains and technologies, from web applications to complex backend systems.";
...
export function generateContactAnswer(sectionGroups: Record<string, string[]>): string {
  ...
  Object.entries(sectionGroups).forEach(([section, texts]) => {
    if (section.includes("Contact") || section.includes("Social")) {
      answer += texts
        .map(
          (t) => `${t.replace(/^My /i, "").replace(/^My (.+) is (.+)/i, "my $1: $2")}, or through `
        )
        .join("");
    }
  });
...
export function generateIntroductionAnswer(cvdata: any): string {
  ...
  const yearsOfExperience = Math.floor(
    (currentDate.getTime() - earliestWorkDate.getTime()) / (1000 * 60 * 60 * 24 * 365)
  );
...
export function generateGeneralAnswer(clusteredResults: any[], _question: string): string {
  const introductions = [
