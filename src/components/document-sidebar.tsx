...
import type { DocumentSidebarProps } from "../types/documents";
...
export function DocumentSidebar({
  ...
  useEffect(() => {
    if (selectedDocument) {
      const element = document.querySelector(`[data-cert-id="${CSS.escape(selectedDocument.id)}"]`);
      if (element) {
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="p-4 border-b border-border">
        <h2 className="text-lg font-semibold text-text1">Certificates</h2>
      ...
      <div className="flex-1 overflow-y-auto" onWheel={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div
                  className={`flex-shrink-0 mt-1 ${
                    selectedDocument?.id === document.id
                      ? "border-2 border-accent-primary rounded"
                      : ""
                  }`}
                >
                ...
                <div className="flex-1 min-w-0">
                  <h3
                    className={`text-sm font-medium truncate ${
                      selectedDocument?.id === document.id ? "text-text1" : "text-text1"
                    }`}
                  >
                    {document.name}
                  </h3>
                  ...
                  <div className="flex items-center space-x-4 mt-1 text-xs text-text2">
                    <div className="flex items-center space-x-1">
                      ...
                      <span>
                        {document.reissuedDate
                          ? `Reissued: ${new Date(document.reissuedDate).toLocaleDateString()}`
        ...
        {documents.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 px-4">
            ...
            <h3 className="text-sm text-text2 font-medium mb-2">No Certificates Found</h3>
      ...
      <div className="p-4 border-t border-border">
        <p className="text-xs text-text2">Select a certificate to view content</p>
      </div>
    </div>
  );
}
