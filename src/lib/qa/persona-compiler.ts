...
export function compileProfilePacketFromSources(
  ...
  return {
    ...
    coreIdentity: 'Stub persona core (full PR2 fusion of cvdata + persona MDs in sibling/merged tree)',
    ...
    structuredSnapshot: {},
    ingestDocument: (rawSources || []).map((s) => `# ${s.name}\n${s.content || '(missing in skeleton)'} `).join('\n\n---\n\n') || '# Stub ingest document (PR2 supplies real in full tree)',
    toolSystemPrompt: 'You are Peramanathan Sathyamoorthy answering in first person (stub). Use the provided tools (Collections-backed) for every factual detail. Warm, professional, with light sparkle per Q6.',
  };
}
