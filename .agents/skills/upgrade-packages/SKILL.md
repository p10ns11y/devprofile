...
description: >-
  ...
  deps, or migrate to a new major of Next, React, TypeScript, Tailwind, or ESLint.
---
...
3. **Framework libs are worth major upgrades** — `next`, `react`, `react-dom`, `typescript`, `tailwindcss`, `eslint` — but require changelog review, codemods, and code fixes; never bump only the version pin.
4. **Keep related packages aligned** — React + types + DOM; Next + `@next/env` override; ESLint + `@eslint/*` plugins.
...
**Tier C “hard deps”** (treat as framework): `next`, `react`, `react-dom`, `typescript`, `tailwindcss`, `eslint`, `@playwright/test`, and UI stacks tightly coupled to React (`motion`, `react-pdf`, `@react-pdf/renderer`).
...
- Fix remaining `tsc` and ESLint errors.
- Update `next.config.*`, `postcss.config.*`, `eslint.config.*` per migration guides.
...
| Transitive via upgradable parent | Bump parent (e.g. `eslint-config-next`) so it depends on a trusted release |
...
| `typescript` | `^6.0.2` — major bumps need full `tsc` pass |
| `eslint` | Flat config in `eslint.config.js`; use `@eslint/migrate-config` on major jumps |
| `tailwindcss` | v4 + `@tailwindcss/postcss` — use `@tailwindcss/upgrade` for major migrations |
| `motion` | Framer Motion successor — check [motion migration](https://motion.dev/) when major bumping |
| `@huggingface/transformers` | Heavy native deps — run [audit-allow-builds](../audit-allow-builds/SKILL.md) after upgrades (sharp, onnxruntime-node, protobufjs) |
| Overrides | Security + trust pins (`semver`, `eslint-import-resolver-typescript`, ONNX) — use `pnpm why` before adding |
